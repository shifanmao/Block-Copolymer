function val=gamq2wlc(M,NM,LAM,k)
...
for j=1:length(k)
    if k(j)*sqrt(r2(NM))<1e-2
        ...
        for I=1:NR
            ...
            Z1=Z0*LAM;
            valeq=2*M*(Z0/R(I)^2);
            valne=4/R(I)^2*Z1*(Z1.^M-M*Z1+M-1)/(1-Z1)^2*(cosh(R(I)*NM)-1);
