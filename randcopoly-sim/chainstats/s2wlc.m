function [valaa,valab,valba,valbb]=s2wlcbac(N,NM,FA,LAM,k,d,ORDmax,ORD,ResLayer)
...
for j=1:length(k)
    ...
    valeq=2*N*(NM/j0-dj0/j0^2);
    
    valaa(j,:)=valaa(j,:)+FA*valeq;
    valbb(j,:)=valbb(j,:)+(1-FA)*valeq;
    ...
    for I=1:NR
        ...
        Z1=Z0*LAM;
        valeq=2*N*(Z0/R(I)^2);
        valne1=4/R(I)^2*Z0*(Z0.^N-N*Z0+N-1)/(1-Z0)^2*(cosh(R(I)*NM)-1);        
        valne2=4/R(I)^2*Z1*(Z1.^N-N*Z1+N-1)/(1-Z1)^2*(cosh(R(I)*NM)-1);
        ...
        valaa(j,:)=valaa(j,:)+Residue(I)*(FA*valeq+FA^2*valne1+FA*(1-FA)*valne2);
        valab(j,:)=valab(j,:)+Residue(I)*(FA*(1-FA)*valne1-FA*(1-FA)*valne2);
        valbb(j,:)=valbb(j,:)+Residue(I)*((1-FA)*valeq+(1-FA)^2*valne1+FA*(1-FA)*valne2);
    end
