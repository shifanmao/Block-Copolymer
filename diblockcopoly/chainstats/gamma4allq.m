function val=gamma4allq(N,FA,Q1,Q2,Q3,Q4)
...
%         Q4=Q4./norm(Q4)*k(j);
        ...
        s2inv1=s2inverse(N,FA,norm(Q1));
        s2inv2=s2inverse(N,FA,norm(Q2));
        s2inv3=s2inverse(N,FA,norm(Q3));
        s2inv4=s2inverse(N,FA,norm(Q4));
        ...
        for I = 1:length(M)
            val(j) = val(j) + g4(M(I,1),M(I,2),M(I,3),M(I,4))*...
                    (s2inv1(M(I,1),1)-s2inv1(M(I,1),2))*...
                    (s2inv2(M(I,2),1)-s2inv2(M(I,2),2))*...
                    (s2inv3(M(I,3),1)-s2inv3(M(I,3),2))*...
                    (s2inv4(M(I,4),1)-s2inv4(M(I,4),2));
        end
%    end
end
