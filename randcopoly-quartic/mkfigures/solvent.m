...
for LAM = 0
    ...
    for ii = 1:length(CHIABV)
        ...
        [EIG1,EIG2,EIGV1,EIGV2,~,~]=gamma2_solvent(N,NM,LAM,FA,KV,CHI,PHIP);

        V1 = [FA,FB];V1 = V1/norm(V1);
        V2 = [FA,-FB];V2 = V2/norm(V2);
        COMP1 = sum(repmat(V1,length(EIGV1),1).*EIGV1,2);
        COMP2 = sum(repmat(V1,length(EIGV2),1).*EIGV2,2);
        EIG = sqrt(COMP1.^2.*EIG1.^2+COMP2.^2.*EIG2.^2);
        figure(1);loglog(KV*RM, 1./EIG/NM, '-', 'Linewidth', 1.5,'color',[COL 0 1-COL]);
        
        COMP1 = sum(repmat(V2,length(EIGV1),1).*EIGV1,2);
        COMP2 = sum(repmat(V2,length(EIGV2),1).*EIGV2,2);
        EIG = sqrt(COMP1.^2.*EIG1.^2+COMP2.^2.*EIG2.^2);
        figure(2);loglog(KV*RM, 1./EIG/NM, '-', 'Linewidth', 1.5,'color',[COL 0 1-COL]);
    end
...
%     t = linspace(0,2*pi,1000);x = sin(t);y = cos(t);
%     set(gca,'fontsize',18)
