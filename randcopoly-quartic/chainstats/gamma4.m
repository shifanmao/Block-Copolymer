function val=gamma4(N,NM,LAM,FA,k,Q1,Q2,Q3,Q4)
...
if norm(Q1+Q2+Q3+Q4)>=MIN
    ...
    for j=1:length(k)
        ...
        elseif NM<=1e-2  % Rigid rod limit
            ...
            s32 = s3rr(N,NM,LAM,FA,Q1,Q3,-Q1-Q3);
            s33 = s3rr(N,NM,LAM,FA,Q1,Q4,-Q1-Q4);
        else
            % Worm-like chain correlations
            s4 = s4wlc(N,NM,LAM,FA,Q1,Q2,Q3,Q4);
            s31 = s3wlc(N,NM,LAM,FA,Q1,Q2,-Q1-Q2);
            s32 = s3wlc(N,NM,LAM,FA,Q1,Q3,-Q1-Q3);
            s33 = s3wlc(N,NM,LAM,FA,Q1,Q4,-Q1-Q4);
        end
        s2inv=s2inverse(N,NM,LAM,FA,k(j));
        s21inv=s2inverse(N,NM,LAM,FA,Q12);
        s22inv=s2inverse(N,NM,LAM,FA,Q13);
        s23inv=s2inverse(N,NM,LAM,FA,Q14);
