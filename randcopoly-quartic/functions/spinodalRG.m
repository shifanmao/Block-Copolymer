function [chit,phase]=spinodalRG(N,CV,NMV,LAM,FAV)
...
% results to return
chit=zeros(length(FAV),length(NMV));     % renormalized spinodal
phase=zeros(length(FAV),length(NMV));     % renormalized spinodal
...
for ii=1:length(FAV)
    ...
    for jj=1:length(NMV)
        ...
        for kk=1:length(CV)
            ...
            if abs(FA-0.5)<1e-2
                chit(ii,jj,kk)=spinodalfh(N,C,NM,d2gam2(ii),gam3(ii),gam4(ii),ks(ii),chis(ii),1);
                phase(ii,jj,kk)=1;
            else
                % find renormalized spinodal of each phase
                chi1=spinodalfh(N,C,NM,d2gam2(ii),gam3(ii),gam4(ii),ks(ii),chis(ii),1);
                chi3=spinodalfh(N,C,NM,d2gam2(ii),gam3(ii),gam4(ii),ks(ii),chis(ii),3);
                chi6=spinodalfh(N,C,NM,d2gam2(ii),gam3(ii),gam4(ii),ks(ii),chis(ii),6);
...
function chit=spinodalfh(N,C,NM,d2gam2,gam3,gam4,ks,chis,n)
% calculate renormalized spinodal from FH theory    
    % calculate constant (estimate local second-order derivative)
    alpha=power(d2gam2/2*NM/r2(NM),1/2);
    d=r2(NM)*ks^2/(4*pi);
    miu=NM*gam3/power(alpha,3);
    lam=NM*gam4(1)/power(alpha,4);
