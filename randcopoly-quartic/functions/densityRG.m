function [chis,chit,CHIV,Smf,Sfh,sinvmf,sinvfh]=densityRG(N,C,NM,LAM,FA)
...
[chis,ks,d2gamma2]=spinodal(N,NM,LAM,FA);
...
for ii = 1:length(CHIV)
    CHI = CHIV(ii)*chis;
    Smf(ii)=densitymf(N,NM,LAM,FA,ks,CHI);
    Sfh(ii)=densityfh(N,C,NM,LAM,FA,ks,ks,CHI,d2gamma2);
end
...
gam2 = gamma2(N,NM,LAM,FA,ks,CHI);
