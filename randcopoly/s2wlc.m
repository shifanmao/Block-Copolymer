function [val]=s2wlc(N,NM,LAM,k,d,ORDmax,ORD,ResLayer)
% Calculate the Fourier transform of the Green function
% for the wormlike chain in d-dimensions, with randomly
% distributed chemical identities
% Usage ::
%   [val]=s2wlc(N,NM,LAM,k,d,ORDmax,ORD,ResLayer)
% Inputs ::
...
for j=1:length(k)
    ...
    for I=1:NR
        ...
        Z1=Z0*LAM;
        valeq=(NM^2)*Z0/R(I)^2;
        valne=(2*NM^2*Z1./N).*(Z1.^N-N*Z1+N-1)/(1-Z1)^2*(cosh(R(I)*NM)-1)/R(I)^2;
